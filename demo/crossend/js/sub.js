...
KISSY.use(__IMPORT, function(S, Xtpl, Flipsnap, Hammer, Cookie, U) {
    ...
    function renderPhone(model) {
        ...
        function domRender(){
            ...
            var $loading = S.one('.subject-loading'),
                $tip = S.one('#J_Tip');
            ...
            fsSlide.element.addEventListener('fstouchend', function(ev) {
                var curView = parseInt(U.getView(), 10), // NaN的情况
                    newView = ev.newPoint;
            ...
            S.each(S.query('.J_ListItemGo'), function(item, i) {
                ...
                Hammer(item).on('tap',function(ev) {
                    ...
                    setTimeout(function(){
                        ...
                        if(location.search.split('=')&&location.search.split('=')[1]*1 === id){
                            //同一个目录的话就不用跳了……
                        } else{
                            phoneGlobal.listOpenMark = true;
                            location.hash = '!0';
        ...
        function activeBind(){
            ...
            var J_ATBox = S.get('#J_ATBox'),//获取小猫
                ...
                $loading = S.one('.subject-loading'),//loading加载状态
                velocity = 0.2,
                barStatus = {
                    status : 0
                };
    ...
    function renderPc(model) {
        ...
        var D = S.DOM;
        S.use('gallery/autoResponsive/1.1/index', function(S, AutoResponsive) {
            var auto = new AutoResponsive({
            ...
            auto.on('beforeSort',function(d){
                D.css('.j-subject-container',{
                    opacity:0
                });
            });
            auto.on('afterSort',function(d){
                setTimeout(function(){
                    D.css('.j-subject-container',{
                        opacity:1
                    });
